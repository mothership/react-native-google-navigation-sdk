...
//        self.reactOnBearingChange?()
    ...
    private func updateCameraIfCenterCoordinateAndZoomSet() {
        print("Lat \(self.reactCenterLatitude) Lng \(self.reactCenterLongitude) zoom \(self.reactZoom) scroll \(self.reactScrollEnabled) include \(self.reactIncludeUserLocationInMapBounds) Lat \(self.reactInitialUserLocationLatitude) Lng \(self.reactInitialUserLocationLongitude) Tag \(self.reactTag) Polyline \(self.reactPolyline?.substring(to: 50)) Markers \(self.reactMarkers?.count)")
